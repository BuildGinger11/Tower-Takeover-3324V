...
bool open = false;
void Open_Intakes(int Position)
{
  if(Position == Open)
  {
    while(OpenLimit.get_value() == 0)
    {
      Index.move(-127);
    }
  }
...
void liftstuff()
{
  ...
  if(master.get_digital(DIGITAL_R2) && OpenLimit.get_value() == 0)
  {
    while(OpenLimit.get_value() == 0)
    {
      intake(-127);
  ...
  if(master.get_digital(DIGITAL_R2) && OpenLimit.get_value() == 1)
  {
    while(OpenLimit.get_value() == 1)
    {
      intake(-127);
...
void liftstuff()
{
  ...
  else if(Auto == true && Stopper.get_value() == 0)
  {
    Lift(100);
  }
  else if(Stopper.get_value() == 1)
  {
    Auto = false;
    Lift(0);
  }
